...
function Fsolve(init_params,params_in)
    ...
    # 计算上边界条件 - 硬约束
    Threads.@threads for i in eachindex(x)
        Loss_bc_u_up[i] = u(x[i], y[ny]) - v_slip
...
#@time sol = solve(prob, LeastSquaresOptimJL(:lm,linsolve = :qr, autodiff = :central))
...
p1 = plot(plot_x, plot_y, u_num, linetype =:contourf, title = "u (m/s)", xlabel = "x (m)", ylabel = "y (m)")
p2 = plot(plot_x, plot_y, v_num, linetype =:contourf, title = "v (m/s)", xlabel = "x (m)", ylabel = "y (m)")
p3 = plot(plot_x, plot_y, velocity_num, linetype =:contourf, title = "velocity (m/s)", xlabel = "x (m)", ylabel = "y (m)")
p4 = plot(plot_x, plot_y, p_num, linetype =:contourf, title = "p (Pa)", xlabel = "x (m)", ylabel = "y (m)")
