...
include("analysis_Burggraf.jl")

nx = 9
ny = 9
num_layers = 2 # output_dim
num_neurals = 5 # neurals_scale
## 参数设定
input_dims = 2 # x,y
output_dims = 1 # u,v,p
# 常数
...
function Fsolve(init_params,params_in)
    ...
    # 计算压力点约束
    Loss_bc_p_points[1] = model_p([0,0])[1] - 0.0
    ...
    Threads.@threads for i in eachindex(x)
        @inbounds begin
            ...
            # 计算上边界条件 - 硬约束
            Loss_bc_u_up[i] = up[i, ny] - U_slipx(x[i])
    ...
    Threads.@threads for i in 1:nx
        Threads.@threads for j in 1:ny# 评估点值
            ...
            Loss_momenty[i, j] = (U*Vx_up + V*Vy_up) + Py/rho - mu/rho*(Vxx + Vyy) - fex(x[i],y[j],Re) # V动量方程
            #Loss_mass[i, j] = rho*(Ux + Vy) # 连续性方程
            Loss_mass[i, j] = (Pxx + Pyy)/rho + (Ux^2 + Vy^2 + 2*Uy*Vx) # 压力泊松方程
        end
    end
...
v_ANA = ones(plot_nx, plot_ny)
velocity_ANA = ones(plot_nx, plot_ny)
p_ANA = ones(plot_nx, plot_ny)
...
for i in eachindex(plot_x)
    for j in eachindex(plot_y)
        ...
        u_ANA[i, j] = u_ana(plot_x[i], plot_y[j])
        v_ANA[i, j] = v_ana(plot_x[i], plot_y[j])
        velocity_ANA[i, j] = (u_ANA[i, j]^2 + v_ANA[i, j]^2)^0.5
        p_ANA[i, j] = p_ana(plot_x[i], plot_y[j], Re)
...
p_ANA = p_ANA'
CSV.write("Possion_Burggraf_u_num_steady.csv", DataFrame(u_num, :auto))
CSV.write("Possion_Burggraf_v_num_steady.csv", DataFrame(v_num, :auto))
CSV.write("Possion_Burggraf_velocity_num_steady.csv", DataFrame(velocity_num, :auto))
CSV.write("Possion_Burggraf_p_num_steady.csv", DataFrame(p_num, :auto))
